...
@section('content')
    <div class="page-title">
       {{trans('labels.my_job')}}&nbsp;&nbsp;
        <a href="{{url('/employer/job/create')}}" class="btn btn-primary btn-sm float-right">{{trans('labels.post_new_job')}}</a>
    </div>
            ...
            <table class="table table-hover b">
                ...
                @foreach($jobs as $job)
                    <tr>
                        ...
                        <td>{{$job->location}}</td>
                        <td>
                            <a class="btn btn-sm btn-info" href="{{url('/employer/job/edit/'.$job->id)}}" title="Edit">Edit</a>&nbsp;&nbsp;
